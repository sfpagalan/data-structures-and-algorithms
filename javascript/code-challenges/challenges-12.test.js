...
const replaceZeros = (string) => {
  const regex = /0/g;
  const replacedString = string.replace(regex, 'zero');
...
const hasNumber = (string) => {
  const regex = /^[a-zA-Z]+.*\d+/;  return regex.test(string);
};
...
const findTagNames = elements => {
  const regex = /<\/(\w+)>/g;
  const closingTags = elements.flatMap(element => {
