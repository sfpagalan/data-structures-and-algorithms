...
const getStatName = (arr, minBaseStat) => {
  const filteredStats = arr.filter(item => item.baseStat > minBaseStat);
  return filteredStats.map(item => item.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  ...
  const evenOddStrings = numericValues.map(number => (number % 2 === 0) ? 'even' : 'odd');
  return evenOddStrings;
};
