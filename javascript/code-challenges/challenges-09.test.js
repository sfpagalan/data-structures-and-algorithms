...
const updateNumbers = (obj) => {
  const formattedData = Object.keys(obj).map(name => `${name}: ${obj[name]}`);
...
const hasChildrenValues = (arr, character) => {
  for (const data of arr) {
    if (data.name === character && data.children && data.children.length > 0) {
      return true;
    }
  }
  return false;
...
const houseSize = (arr) => {
  ...
  for (const character of arr) {
    ...
    const existingHouse = sizes.find(house => house.house === houseName);

    if (existingHouse) {
      existingHouse.members++;
    } else {
      sizes.push({ house: houseName, members: 1 });
    }
  }
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  for (const house of arr) {
    const survivingMembers = house.members.filter(member => !deceasedSpouses.includes(member.name));
    const survivingSpouse = house.spouse && !deceasedSpouses.includes(house.spouse) ? 1 : 0;
    const totalSurvivingMembers = survivingMembers.length + survivingSpouse;
    survivors.push({ house: house.house, members: totalSurvivingMembers });
  }
  return survivors;
};
