...
const removeThree = (idx, arr) => {
  if (idx < 0 || idx + 2 >= arr.length) {
    throw new Error("Invalid index or insufficient items to remove.");
  }
...
const foodName = listFoods(gruffaloCrumble);
...
const splitFoods = (recipe) => {
  ...
  for (const ingredient of recipe.ingredients) {
    const foodNames = ingredient.split(' ').slice(2).join(' ');
    result.push(foodNames);
  }

  return result;
};

const foodNames = splitFoods(gruffaloCrumble);
...
const removeVowels = (str) => {
  ...
  const result = str.replace(new RegExp(`[${vowels}]`, 'g'), '');
...
const extractVowels = (str) => {
  ...
  return [resultString, removedVowels.split('').sort().join('')];
};
