...
const count = (target, input) => {
  const totalCount = input.reduce((accumulator, subArray) => {
    const countInSubArray = subArray.filter(item => item === target).length;
    return accumulator + countInSubArray;
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  const result = input.map(subArray => {
    ...
    const powerOfTwoArray = filteredArray.map(item => Math.pow(2, item));
...
let findMaleAndFemale = (data) => {
  ...
  const combinedNames = characterNames.join(' and ');
